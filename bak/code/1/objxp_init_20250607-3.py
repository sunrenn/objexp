...
def classify_member(obj, member_name):
    ...
    try:
        member_value = str(getattr(obj, member_name))[:100]  # 限制值的长度
    except:
        member_value = '<unavailable>'
...
def ox(
    ...
    for member in sorted(members):
        ...
        if show_inner_members == 'none' and member.startswith('_'):
            continue
        if show_inner_members == '_' and member.startswith('__'):
            continue
        if show_inner_members == '__' and member.startswith('_') and not member.startswith('__'):
            continue
    ...
    for category in sorted(classified_members.keys()):
        ...
        # 添加分类标题
        result_content_text += f"\n\033[1;40;34m {category.upper()} \033[0m\n"
